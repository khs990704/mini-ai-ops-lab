...
- 일별 작업 기록: `docs/work-logs/YYYY-MM-DD-DayN.md`
- 기술 위키: `docs/wiki/`
...
세부 Day별 계획은 `docs/daily-codex-workflow.md`를 따른다. 하루 작업은 독립적으로 검증 가능한 작은 단계로 나누며, 각 단계는 `한 단계 구현 -> 검증 결과 공유 -> 사용자 검수 -> 다음 단계 승인` 순서로 진행한다. 최초 계획 승인은 첫 단계만 허용하며 Codex는 다음 단계를 자동으로 이어서 수행하지 않는다.
...
| 3 | 20분 | 작업 검토와 Q&A 후 오늘 사용한 개념을 `docs/wiki/`에 분류해 정리 |
...
매일 구현과 검증 결과를 사용자가 모두 확인하고 관련 Q&A가 끝난 뒤, 사용자의 기록 승인에 따라 `docs/work-logs/YYYY-MM-DD-DayN.md` 형식의 새 파일을 만든다. 작업 일지는 사용자가 프로젝트를 수행하고 학습한 관점으로 작성하며, 사용자가 표현하지 않은 감정이나 경험을 임의로 만들지 않는다. 기술 개념과 유용한 질문은 Codex가 적절한 `docs/wiki/` 카테고리를 선택해 반영한다.
...
단계별 진행 및 검수: 1단계 학습 함수 구현과 metric 출력 검증 후 사용자 검수 완료
코드 의도와 주요 주석: 실행 진입점은 학습 흐름을 조정하고, 저장 함수는 run별 artifact 경로를 분리한다.
실행한 명령과 목적: `python src/run_job.py` - 학습 작업을 실행하고 run log와 artifact 생성을 확인
검증 결과: logs/runs.jsonl에 success 로그가 추가됨
직접 확인 방법: 프로젝트 root에서 같은 명령을 실행하고 success 로그와 모델 파일 생성 확인
